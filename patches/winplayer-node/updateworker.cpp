...
void UpdateWorker::OnOK()
{
	...
	if (currentUpdate.has_value())
	{
		...
		// stamp from capture time, NOT from now(): the worker spent the whole
		// metadata/artwork await between capturing elapsed and reaching OnOK
		jsPosition.Set("when",
					   Napi::Date::New(
						   env,
						   static_cast<double>(currentUpdate->elapsedAtMs)));
